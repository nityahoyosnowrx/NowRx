<section class="interactionchecker">
        ...
        <a class="main-nav-logo" href="https://nowrx.com/" target="_blank"
            ...
            <figure class="sk">
                <img src="<?php echo get_template_directory_uri(); ?>/library/images/NowRx-Logo-Pharmacy.png"
                    class="orbit-image" alt="NowRx Pharmacy Logo" width="150px">
            </figure>
        </a>
    ...
    <div class="disclaimer">
        DISCLAIMER: The information contained herein should NOT be used as a substitute for the advice of an
...
// on keydown input
    ...
    makeRequest('https://rxnav.nlm.nih.gov/REST/approximateTerm.json?term=' + inputText + '&maxEntries=99')
        .then(function(posts) {
            ...
            if(getNameJson.approximateGroup.candidate){
                ...
                getNameJson.approximateGroup.candidate.forEach(function(element) {
                    ...
                    makeRequest('https://rxnav.nlm.nih.gov/REST/rxcui/' + numJs + '.json')
                        .then(function(posts) {
                            let getNameid = posts.response;
                            let getNameJsonid = JSON.parse(getNameid);

                            if (fruits.includes(getNameJsonid.idGroup.rxnormId[0])) {

                            } else {
                                fruits.push(getNameJsonid.idGroup.rxnormId[0]);

                                if (getNameJsonid.idGroup.name) {
                                    $($this).siblings(".rella").find('.autocompleteform').append('<span class="selectnormid" data-normid="' + getNameJsonid.idGroup.rxnormId[0] + '">' + getNameJsonid.idGroup.name + '</span>');
                                }
                            }
                        })
