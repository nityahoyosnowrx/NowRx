<?php get_header(); ?>

<main id="main" class="main-class">



    <section class="resource-sections hrline">
        <div class="container">
            <div class="content">
                <div class="context">
                    <h2 class="title">Customer Resource Center</h2>
                    <p>
                        Everything you need to transform the way you work with Workplace.
                    </p>
                </div>
                <div class="blocks">
                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="light" enable-background="new 0 0 24 24"
                                        height="512" viewBox="0 0 24 24" width="512">
                                        <path
                                            d="m11.861 19.066c-.128 0-.256-.049-.354-.146l-6.427-6.427c-.188-.188-.196-.489-.02-.687l4.817-5.396c3.367-3.939 8.85-6.488 13.631-6.407.267.006.482.221.488.488.111 4.784-2.467 10.264-6.416 13.638l-5.387 4.81c-.094.085-.214.127-.332.127zm-5.74-6.946 5.759 5.759 5.043-4.502c3.606-3.082 6.001-7.994 6.075-12.375-4.381.073-9.292 2.468-12.368 6.066z" />
                                        <path
                                            d="m13.776 22.726c-.048 0-.097-.007-.143-.022-.18-.06-.328-.198-.354-.386l-.686-4.787c-.039-.273.151-.526.424-.566.273-.043.526.15.566.424l.536 3.74c1.752-2.095 2.726-4.751 2.726-7.459 0-.276.224-.5.5-.5s.5.224.5.5c0 3.313-1.342 6.556-3.682 8.895-.1.1-.246.161-.387.161z" />
                                        <path
                                            d="m6.541 11.412c-.023 0-.047-.001-.071-.005l-4.787-.686c-.188-.027-.345-.158-.404-.339-.059-.18-.011-.379.124-.512 2.373-2.373 5.615-3.715 8.928-3.715.276 0 .5.224.5.5s-.224.5-.5.5c-2.721 0-5.391.983-7.464 2.725l3.745.537c.273.04.463.292.424.566-.037.249-.251.429-.495.429z" />
                                        <path
                                            d="m17.133 9.366c-.641 0-1.281-.244-1.768-.731-.974-.975-.974-2.561 0-3.536.975-.975 2.561-.975 3.536 0 .974.975.974 2.561 0 3.536-.488.488-1.128.731-1.768.731zm0-3.998c-.384 0-.769.146-1.061.438-.584.585-.584 1.537 0 2.122.585.584 1.537.583 2.122 0 .584-.585.584-1.537 0-2.122-.293-.291-.677-.438-1.061-.438zm1.414 2.914h.01z" />
                                        <path
                                            d="m.5 24c-.131 0-.258-.051-.354-.146-.13-.13-.178-.321-.125-.497.164-.547 1.633-5.381 2.703-6.451 1.205-1.205 3.166-1.206 4.371 0 1.205 1.205 1.205 3.166 0 4.371-1.07 1.07-5.904 2.539-6.451 2.703-.048.013-.096.02-.144.02zm4.41-6.999c-.536 0-1.071.204-1.479.611-.577.578-1.537 3.159-2.171 5.128 1.968-.634 4.55-1.594 5.127-2.171.815-.815.815-2.142 0-2.957-.407-.407-.942-.611-1.477-.611z" />
                                    </svg>
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                    Get Started
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>
                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Icons" height="512" viewBox="0 0 74 74"
                                        width="512">
                                        <path
                                            d="m57 72h-40a5.006 5.006 0 0 1 -5-5v-60a5.006 5.006 0 0 1 5-5h40a5.006 5.006 0 0 1 5 5v60a5.006 5.006 0 0 1 -5 5zm-40-68a3 3 0 0 0 -3 3v60a3 3 0 0 0 3 3h40a3 3 0 0 0 3-3v-60a3 3 0 0 0 -3-3z" />
                                        <path
                                            d="m53.5 23h-33a2 2 0 0 1 -2-2v-9a2 2 0 0 1 2-2h33a2 2 0 0 1 2 2v9a2 2 0 0 1 -2 2zm-33-11v9h33v-9z" />
                                        <path
                                            d="m52.625 37.292h-5.208a1.71 1.71 0 0 1 -1.709-1.708v-5.209a1.711 1.711 0 0 1 1.709-1.708h5.208a1.711 1.711 0 0 1 1.708 1.708v5.208a1.71 1.71 0 0 1 -1.708 1.709zm-4.917-2h4.625v-4.625h-4.625z" />
                                        <path
                                            d="m39.6 37.292h-5.2a1.71 1.71 0 0 1 -1.708-1.708v-5.209a1.711 1.711 0 0 1 1.708-1.708h5.2a1.711 1.711 0 0 1 1.708 1.708v5.208a1.71 1.71 0 0 1 -1.708 1.709zm-4.917-2h4.625v-4.625h-4.62z" />
                                        <path
                                            d="m26.583 37.292h-5.208a1.71 1.71 0 0 1 -1.708-1.708v-5.209a1.711 1.711 0 0 1 1.708-1.708h5.208a1.711 1.711 0 0 1 1.709 1.708v5.208a1.71 1.71 0 0 1 -1.709 1.709zm-4.916-2h4.625v-4.625h-4.625z" />
                                        <path
                                            d="m39.6 50.313h-5.2a1.711 1.711 0 0 1 -1.712-1.713v-5.2a1.711 1.711 0 0 1 1.712-1.712h5.2a1.711 1.711 0 0 1 1.713 1.712v5.2a1.711 1.711 0 0 1 -1.713 1.713zm-4.917-2h4.625v-4.625h-4.62z" />
                                        <path
                                            d="m26.583 50.313h-5.208a1.711 1.711 0 0 1 -1.708-1.713v-5.2a1.711 1.711 0 0 1 1.708-1.708h5.208a1.711 1.711 0 0 1 1.709 1.708v5.2a1.711 1.711 0 0 1 -1.709 1.713zm-4.916-2h4.625v-4.625h-4.625z" />
                                        <path
                                            d="m52.622 63.33h-5.2a1.711 1.711 0 0 1 -1.708-1.708v-18.222a1.711 1.711 0 0 1 1.708-1.708h5.2a1.711 1.711 0 0 1 1.708 1.708v18.222a1.711 1.711 0 0 1 -1.708 1.708zm-4.912-2h4.62v-17.64h-4.62z" />
                                        <path
                                            d="m39.6 63.333h-5.2a1.711 1.711 0 0 1 -1.708-1.708v-5.208a1.71 1.71 0 0 1 1.708-1.709h5.2a1.71 1.71 0 0 1 1.708 1.708v5.208a1.711 1.711 0 0 1 -1.708 1.709zm-4.917-2h4.625v-4.625h-4.62z" />
                                        <path
                                            d="m26.583 63.333h-5.208a1.711 1.711 0 0 1 -1.708-1.708v-5.208a1.71 1.71 0 0 1 1.708-1.708h5.208a1.71 1.71 0 0 1 1.709 1.708v5.208a1.711 1.711 0 0 1 -1.709 1.708zm-4.916-2h4.625v-4.625h-4.625z" />
                                    </svg>
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                    Drug Interactions Checker
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>
                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                        version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 512.001 512.001"
                                        style="enable-background:new 0 0 512.001 512.001;" xml:space="preserve">
                                        <g>
                                            <g>
                                                <path
                                                    d="M351.236,0c-50.526,0-96.567,27.786-120.155,72.514c-1.933,3.664-0.529,8.2,3.135,10.132    c3.664,1.935,8.2,0.529,10.132-3.135C265.333,39.719,306.29,15,351.236,15c66.586,0,120.758,54.172,120.758,120.758    s-54.172,120.758-120.758,120.758c-22.687,0-44.793-6.324-63.928-18.29c-3.514-2.195-8.14-1.129-10.336,2.383    c-2.196,3.512-1.129,8.14,2.383,10.336c21.522,13.458,46.378,20.571,71.88,20.571c74.857,0,135.758-60.901,135.758-135.758    C486.994,60.901,426.093,0,351.236,0z" />
                                            </g>
                                        </g>
                                        <g>
                                            <g>
                                                <path
                                                    d="M276.298,347.667l-60.109-27.563c-3.022-1.386-4.976-4.43-4.976-7.756v-19.905c18.055-11.799,31.25-30.658,35.407-53.137    c0.006-0.007,0.013-0.015,0.02-0.022c2.138,0.128,4.3-0.063,6.425-0.567c9.911-2.346,17.104-11.851,17.104-22.601v-5.743    c0-7.948-3.99-15.09-10.151-19.21c1.84-7.199,2.781-14.566,2.781-21.983c0-32.57-17.645-61.252-44.225-77.672    c-0.158-28.384-23.291-51.427-51.711-51.427c-28.419,0-51.552,23.044-51.71,51.427c-26.581,16.42-44.226,45.102-44.226,77.673    c0,7.396,0.936,14.744,2.765,21.924c-6.158,4.099-10.135,11.271-10.135,19.356v5.656c0,10.75,7.193,20.255,17.104,22.601    c2.126,0.503,4.287,0.695,6.426,0.566c0.007,0.007,0.014,0.015,0.02,0.022c4.157,22.479,17.353,41.339,35.408,53.137v19.905    c0,3.326-1.953,6.37-4.976,7.756l-60.11,27.563c-20.719,9.501-33.707,31.17-32.32,53.921l6.305,103.369    c0.243,3.979,3.545,7.043,7.479,7.043c0.153,0,0.308-0.005,0.463-0.014c4.134-0.252,7.282-3.809,7.029-7.943L40.08,400.675    c-0.575-9.438,2.24-18.619,7.551-26.062l20.446,24.534c6.589,7.908,10.218,17.931,10.218,28.224V504.5c0,4.143,3.358,7.5,7.5,7.5    c4.142,0,7.5-3.357,7.5-7.5v-77.128c0-13.795-4.864-27.229-13.695-37.827l-21.133-25.359c1.651-1.08,3.39-2.048,5.213-2.884    l35.933-16.477c10.241,27.934,37.015,46.934,67.248,46.934c30.33,0,57.014-18.972,67.246-46.935l35.935,16.478    c1.823,0.836,3.563,1.804,5.213,2.884l-21.133,25.359c-8.832,10.598-13.695,24.031-13.695,37.827V504.5c0,4.143,3.358,7.5,7.5,7.5    c4.142,0,7.5-3.357,7.5-7.5v-77.128c0-10.293,3.629-20.317,10.219-28.224l20.446-24.534c5.311,7.444,8.126,16.624,7.551,26.062    l-6.305,103.368c-0.252,4.135,2.895,7.691,7.029,7.943c0.155,0.009,0.31,0.014,0.463,0.014c3.934,0,7.236-3.064,7.479-7.043    l6.305-103.369C310.006,378.837,297.017,357.168,276.298,347.667z M166.865,55.081c17.424,0,32.042,12.203,35.782,28.509    c-11.063-4.292-23.139-6.661-35.782-6.661c-12.642,0-24.718,2.369-35.781,6.66C134.824,67.284,149.441,55.081,166.865,55.081z     M101.844,236.498c-1.355-7.378-8.193-12.741-15.551-12.194c-0.707,0.051-1.44-0.01-2.174-0.184    c-3.169-0.75-5.56-4.191-5.56-8.004v-5.656c0-3.832,2.579-7.287,5.871-7.864c2.767-0.487,5.425,0.531,7.104,2.722    c1.397,1.822,3.26,3.125,5.392,3.77c3.114,0.939,6.55,0.388,9.19-1.477c6.095-4.302,11.262-9.76,15.356-16.22    c2.217-3.499,1.179-8.132-2.32-10.35c-3.499-2.217-8.132-1.178-10.35,2.32c-2.334,3.683-5.126,6.919-8.329,9.656    c-3.422-3.013-7.676-4.881-12.187-5.393c-1.559-6.042-2.359-12.224-2.359-18.444c0-42.596,36.308-77.251,80.936-77.251    c44.628,0,80.936,34.655,80.936,77.251c0,6.212-0.797,12.384-2.352,18.418c-2.204,0.157-4.458,0.523-6.522,1.072l-6.068-4.99    c-2.902-2.385-6.541-3.579-10.249-3.359c-6.757,0.398-24.649,0.867-44.829-3.391c-24.939-5.261-44.043-15.659-56.782-30.905    c-2.656-3.179-7.386-3.602-10.564-0.946c-3.179,2.656-3.602,7.386-0.946,10.564c14.949,17.891,36.884,29.991,65.196,35.964    c21.841,4.607,41.24,4.126,48.69,3.694l9.352,7.69c2.422,1.991,5.824,2.268,8.533,0.691c1.356-0.759,5.975-1.482,8.02-1.073    c3.358,0.668,5.891,4.005,5.891,7.763v5.743c0,3.813-2.39,7.254-5.56,8.004c-0.734,0.174-1.464,0.234-2.172,0.184    c-7.361-0.536-14.197,4.814-15.553,12.193c-5.763,31.378-33.108,54.152-65.021,54.152    C134.952,290.649,107.607,267.875,101.844,236.498z M166.865,376.758c-24.303,0-45.774-15.525-53.558-38.211l10.487-4.809    c8.337-3.822,13.724-12.219,13.724-21.391v-12.2c9.144,3.551,19.048,5.502,29.348,5.502s20.204-1.951,29.349-5.502v12.2    c0,9.172,5.387,17.568,13.724,21.391l10.494,4.812C212.653,361.262,191.245,376.758,166.865,376.758z" />
                                            </g>
                                        </g>
                                        <g>
                                            <g>
                                                <path
                                                    d="M405.595,107.846c-4.382-21.026-21.453-38.097-42.479-42.479c-16.769-3.494-33.877,0.588-46.933,11.203    c-13.056,10.615-20.544,26.345-20.544,43.156c0,4.142,3.358,7.5,7.5,7.5c4.142,0,7.5-3.358,7.5-7.5    c0-12.276,5.47-23.764,15.007-31.518c9.537-7.754,22.079-10.727,34.409-8.156c15.272,3.183,27.672,15.583,30.855,30.855    c4.418,21.199-7.698,41.735-28.182,47.768c-11.36,3.346-18.993,13.201-18.993,24.522v0.658c0,4.142,3.358,7.5,7.5,7.5    c4.142,0,7.5-3.358,7.5-7.5v-0.658c0.001-4.611,3.309-8.683,8.232-10.134C395.015,164.802,411.622,136.765,405.595,107.846z" />
                                            </g>
                                        </g>
                                        <g>
                                            <g>
                                                <path
                                                    d="M351.316,208.419h-0.08c-4.142,0-7.46,3.358-7.46,7.5c0,4.142,3.398,7.5,7.54,7.5c4.142,0,7.5-3.358,7.5-7.5    C358.816,211.777,355.458,208.419,351.316,208.419z" />
                                            </g>
                                        </g>

                                    </svg>
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                    Frequently Asked Questions
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>
                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1"
                                        enable-background="new 0 0 512 512" height="512" viewBox="0 0 512 512"
                                        width="512">
                                        <g>
                                            <path
                                                d="m512 349.934v-320.705h-512v320.705zm-433.002-30c6.537-35.07 31.608-58.421 64.758-58.421 34.234 0 60.425 25.359 65.166 58.421zm81.073-98.69c-4.281 4.354-10.078 6.752-16.321 6.752-11.721 0-22.339-9.278-22.339-21.323 0-11.758 10.021-21.323 22.339-21.323 12.323 0 22.35 9.565 22.35 21.323 0 5.426-2.15 10.609-6.029 14.571zm-129.071 98.69v-260.705h450v260.705h-241.872c-3.939-34.861-23.047-65.151-55.607-79.894 8.128-9.312 12.579-21.045 12.579-33.367 0-28.3-23.484-51.323-52.35-51.323-28.859 0-52.339 23.023-52.339 51.323 0 12.375 4.479 24.146 12.664 33.468-29.405 13.727-50.212 42.811-55.454 79.793z" />
                                            <path
                                                d="m512 416.562h-339.772c-6.523-21.398-26.868-36.21-49.153-36.21-22.937.068-42.385 15.332-48.806 36.21h-74.269v30h74.252c6.519 21.35 26.847 36.21 49.122 36.21 22.169-.065 42.335-15.014 48.822-36.21h339.804zm-367.55 14.983c-.012 11.549-9.629 21.192-21.163 21.227-11.57 0-21.3-9.566-21.268-21.21 0-11.66 9.485-21.175 21.146-21.21 11.577 0 21.282 9.581 21.285 21.193z" />
                                            <path
                                                d="m305.755 110.864v143.52l143.521-71.76zm30 48.541 46.438 23.219-46.438 23.219z" />
                                        </g>
                                    </svg>
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                    Tutorials

                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>
                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <svg xmlns="http://www.w3.org/2000/svg" height="512" viewBox="0 0 128 128"
                                        width="512">
                                        <g>
                                            <path
                                                d="m78.777 37.021a14.777 14.777 0 1 0 -14.777 14.779 14.795 14.795 0 0 0 14.777-14.779zm-26.054 0a11.277 11.277 0 1 1 11.277 11.279 11.29 11.29 0 0 1 -11.277-11.279z" />
                                            <path
                                                d="m123.328 121.069-14.266-37.4a1.751 1.751 0 0 0 -1.635-1.126h-27c.165-.269.329-.53.494-.8 10.389-17.2 15.617-32.246 15.542-44.714a32.464 32.464 0 0 0 -64.928-.011c-.075 12.479 5.153 27.527 15.542 44.725.165.273.329.534.494.8h-27a1.751 1.751 0 0 0 -1.635 1.126l-14.264 37.4a1.748 1.748 0 0 0 1.635 2.374h115.386a1.748 1.748 0 0 0 1.635-2.374zm-88.292-84.048a28.964 28.964 0 1 1 57.928.01c.15 24.858-23.09 55.517-28.964 62.869-5.874-7.349-29.115-38-28.964-62.879zm27.631 66.779a1.75 1.75 0 0 0 2.666 0 185.716 185.716 0 0 0 12.9-17.759h27.987l2.24 5.875-54.691 19.451-19.494-25.329h15.49a185.716 185.716 0 0 0 12.902 17.762zm-8.959 11.3h.01l32.627-11.6 12.655 16.443h-58.9zm-31.93-29.062h8.08l20.442 26.562-20.643 7.342h-20.81zm81.643 33.905-13.609-17.682 19.9-7.077 9.443 24.759z" />
                                        </g>
                                    </svg>
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                    Locations
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>
                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                        version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 511.874 511.874"
                                        style="enable-background:new 0 0 511.874 511.874;" xml:space="preserve">
                                        <g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M490.114,12.813H132.568c-12.012,0.014-21.746,9.748-21.76,21.76v98.62l-16.35-24.525     c-7.484-11.289-22.535-14.676-34.133-7.68l-33.638,20.224c-11.016,6.464-19.097,16.946-22.545,29.244     c-12.271,44.681-3.166,121.66,109.824,234.667C203.821,474.885,270.816,499.06,316.99,499.06     c10.69,0.049,21.339-1.34,31.659-4.13c12.293-3.448,22.775-11.518,29.252-22.519l20.198-33.673     c6.968-11.589,3.584-26.609-7.68-34.091l-80.546-53.692c-11.049-7.308-25.859-4.905-34.031,5.521l-23.45,30.148     c-2.451,3.226-6.897,4.166-10.445,2.21l-4.463-2.458c-14.686-8.004-32.964-17.971-69.879-54.886     c-3.994-3.994-7.612-7.731-11.008-11.307h333.517c11.982,0.009,21.713-9.676,21.76-21.658V34.573     C511.86,22.561,502.126,12.827,490.114,12.813z M229.318,401.362l4.335,2.381c10.897,6.093,24.614,3.266,32.213-6.639     l23.45-30.148c2.666-3.396,7.49-4.179,11.093-1.801l80.546,53.692c3.659,2.439,4.759,7.321,2.5,11.093l-20.198,33.673     c-4.218,7.233-11.071,12.553-19.123,14.848c-40.337,11.093-110.933,1.707-218.078-105.446S9.56,195.273,20.627,154.97     c2.293-8.051,7.61-14.903,14.839-19.123l33.673-20.207c3.773-2.254,8.652-1.155,11.093,2.5l53.717,80.546     c2.382,3.602,1.599,8.43-1.801,11.093l-30.157,23.458c-9.903,7.597-12.731,21.311-6.639,32.205l2.389,4.335     c8.533,15.65,19.14,35.123,57.805,73.779C194.212,382.213,213.677,392.828,229.318,401.362z M494.808,298.526     c-0.028,2.567-2.127,4.627-4.693,4.608H141.203c-11.083-12.674-20.64-26.604-28.476-41.506l-2.458-4.48     c-1.96-3.54-1.022-7.982,2.202-10.428l30.157-23.458c10.43-8.17,12.833-22.982,5.521-34.031l-20.275-30.43V34.573     c-0.014-1.249,0.476-2.451,1.359-3.334c0.883-0.883,2.085-1.373,3.334-1.359h357.547c1.249-0.014,2.451,0.476,3.334,1.359     c0.883,0.883,1.373,2.085,1.359,3.334V298.526z" />
                                                    <path
                                                        d="M460.725,52.323l-142.618,108.16c-4.035,2.932-9.499,2.932-13.534,0L162.008,52.323     c-3.756-2.849-9.111-2.113-11.959,1.643c-2.849,3.756-2.113,9.111,1.643,11.959l142.583,108.151     c10.144,7.494,23.989,7.494,34.133,0L471.034,65.925c1.805-1.368,2.992-3.398,3.299-5.642c0.307-2.244-0.29-4.518-1.661-6.321     C469.824,50.213,464.478,49.48,460.725,52.323z" />
                                                    <path
                                                        d="M238.517,174.793l-87.415,93.611c-3.214,3.447-3.025,8.848,0.422,12.062c3.447,3.214,8.848,3.025,12.062-0.422     l87.416-93.653c2.888-3.484,2.553-8.617-0.762-11.698C246.924,171.612,241.78,171.656,238.517,174.793z" />
                                                    <path
                                                        d="M384.728,174.793c-3.214-3.447-8.614-3.637-12.062-0.422c-3.447,3.214-3.637,8.614-0.422,12.062l87.39,93.611     c3.214,3.447,8.614,3.637,12.062,0.422c3.447-3.214,3.637-8.614,0.422-12.062L384.728,174.793z" />
                                                </g>
                                            </g>
                                        </g>

                                    </svg>
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                    Contact Us
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </section>



    <section class="resource-sections hrline">
        <div class="container">
            <div class="content">
                <div class="context">
                    <h2 class="title">Tutorials</h2>
                    <p>
                        Internal campaign kits and downloadable playbooks to help you drive employee engagement.
                    </p>
                    <div class="btn-container">
                        <a href="" class="rx-btn">See All Tutorials</a>
                    </div>
                </div>
                <div class="blocks">

                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <img src="https://images.pexels.com/photos/2289407/pexels-photo-2289407.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260" alt="">
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                2021 Employee Engagement Calendar
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>

                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <img src="https://images.pexels.com/photos/2289407/pexels-photo-2289407.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260" alt="">
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                2021 Employee Engagement Calendar
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>

                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <img src="https://images.pexels.com/photos/2289407/pexels-photo-2289407.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260" alt="">
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                2021 Employee Engagement Calendar
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>


                </div>
            </div>
        </div>
    </section>

    <section class="resource-sections hrline">
        <div class="container">
            <div class="content">

                <div class="splitcol">
                    <div class="colde">
                        <img src="https://scontent-lax3-2.xx.fbcdn.net/v/t39.2365-6/148094764_769391693991282_530492947140178937_n.png?_nc_cat=103&ccb=1-3&_nc_sid=ad8a9d&_nc_ohc=bRb_NbRXB1gAX-DgGJ9&_nc_ht=scontent-lax3-2.xx&oh=32bef2bf0e03830ad0d83bcc4ad5127c&oe=60D4C5D4" alt="">
                    </div>
                    <div class="colde">
                        <div class="cntx">
                            <h2 class="title">The Live Video Resource Hub</h2>
                            <p>
                            Tools, training and tips to help you drive employee engagement and build culture using Live video in Workplace.

                            </p>
                            <div class="btn-container">
                                <a href="" class="rx-btn">Explore More Options</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="resource-sections hrline">
        <div class="container">
            <div class="content">
                <div class="context">
                    <h2 class="title">Locations</h2>
                    <p>
                        Internal campaign kits and downloadable playbooks to help you drive employee engagement.
                    </p>
                    <div class="btn-container">
                        <a href="" class="rx-btn">See All Locations</a>
                    </div>
                </div>
                <div class="blocks">

                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <img src="https://images.pexels.com/photos/2289407/pexels-photo-2289407.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260" alt="">
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                Irvine
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>

                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <img src="https://images.pexels.com/photos/2289407/pexels-photo-2289407.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260" alt="">
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                Irvine
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>

                    <!-- block -->
                    <div class="block">
                        <div class="contentblock">
                            <figure>
                                <a href="">
                                    <img src="https://images.pexels.com/photos/2289407/pexels-photo-2289407.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260" alt="">
                                </a>
                            </figure>
                            <h2 class="title">
                                <a href="">
                                Irvine
                                </a>
                            </h2>
                            <p>
                                Learn how to launch Workplace and find user guides to help you get started.
                            </p>
                        </div>
                    </div>


                </div>
            </div>
        </div>
    </section>


    <section class="resources">
        <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>

        <?php if (has_post_thumbnail( $post->ID ) ): ?>

        <?php $image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID),''); ?>

        <a href="<?php the_permalink() ?>" title="<?php the_title_attribute(); ?>">
            <img src="<?php echo $image[0]; ?>" alt="<?php the_title_attribute(); ?>">
        </a>

        <?php endif; ?>

        <h2>
            <a class="featured-post-link-title" href="<?php the_permalink() ?>" rel="bookmark"
                title="<?php the_title_attribute(); ?>">
                <?php the_title(); ?>
            </a>
        </h2>

        <?php endwhile; endif; ?>
    </section>



    <!-- visit faq -->
    <?php
    get_template_part('components/visit-our-faq'); ?>

</main>

<?php get_footer(); ?>