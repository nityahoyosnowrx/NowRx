 ...
 if ( $query->have_posts() ) : 
   while ( $query->have_posts() ) : $query->the_post();
    ...
    ?>
            ...
            <article
                class="section <?php if($postCount == 1 || $postCount == 2 || $postCount == 3 ){ echo "firsttwo"; } ?>">

                <div class="image">
                    <?php if (has_post_thumbnail( $post->ID )): ?>
                    <?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' ); ?>
                    <a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
                        <img class="border-styled" style="" src="<?php echo $image[0]; ?>"
                            alt="<?php echo wp_trim_words(get_the_title(), 8, '...'); ?>">
                    </a>
                    <?php else: ?>
                    <a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
                        <img class="border-styled" style=""
